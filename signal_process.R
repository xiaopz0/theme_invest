signal_process <- function(raw_signal, returns){
  ...
  for(i in 1:length(raw_date)){
    ...
    if(sum(non_nan_index)>1){
      processed_rank <- rank(as.numeric(raw_signal_date)[non_nan_index])
      # demean process gives cross sectional results
      processed_rank <- processed_rank - mean(processed_rank)
...
signal_process_discrete <- function(raw_signal, returns){
  processed_signal <- raw_signal
  processed_signal[,] <- NA
  raw_date <- time(raw_signal)
  return_date <- time(returns)
  for(i in 1:length(raw_date)){
    ...
    if(sum(non_nan_index)>=1){
      ...
      processed_cov <- rolling_asset_cov[non_nan_index,non_nan_index]
      processed_vol <- sqrt(252*processed_rank %*% as.matrix(rolling_asset_cov[non_nan_index,non_nan_index]) %*% processed_rank)
      processed_signal[idate,non_nan_index] <- ((processed_rank / processed_vol > 0) * 2) - 1
    }
  }
  return(processed_signal)
}