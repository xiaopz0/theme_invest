...
#'@export
"quandCleanReturn" <- function(stemCode, start_date=NULL, end_date=NULL, verbose=FALSE, contract=1, ...) {
  require(Quandl)
  if(is.null(start_date)) {start_date <- Sys.Date()-365*1000}
  if(is.null(end_date)) {end_date <- Sys.Date()+365*1000}
  frontCode <- paste0(stemCode, contract)
  backCode <- paste0(stemCode, contract+1)
  ...
  volColname <- colnames(front)[colnames(front)=="Volume"]
  ...
  # calculate front return
  front[,c("O","H","L","C")] = front[,c("O","H","L","C")] / 
  lag(front[,c("O","H","L","C")],-1) - 1
  ...
  # calculate back return
  back[,c("BO","BH","BL","BS")] = back[,c("BO","BH","BL","BS")] / 
  lag(back[,c("BO","BH","BL","BS")],-1) - 1
